...
// BecomeFollower become to follower
func (rn *RawNode) BecomeFollower(term uint64, leader uint64) {
	rn.raft.becomeFollower(term, leader)
}

// NewChanger returns a `confchange.Changer` with cloned stats of current node
...
// PendingConfIndex returns log index of the pending config change
